...
public class MarkdownFileCollector
{
	...
	/**
	 * Returns a list of files with the given extension in the specified directory and its
	 * subdirectories
	 *
	 * @param directoryPath
	 *            the base directory path
	 * @param extension
	 *            the file extension to match (e.g. ".md", ".txt", ".java")
	 * @return list of matching files
	 * @throws IOException
	 *             if an I/O error occurs
	 */
	public static List<File> getFilesWithExtension(String directoryPath, String extension)
		throws IOException
	{
		try (Stream<Path> paths = Files.walk(Paths.get(directoryPath)))
		{
			return paths.filter(Files::isRegularFile)
				.filter(path -> path.toString().endsWith(extension)).map(Path::toFile)
